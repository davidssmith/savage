...
static int units, board, border, corner, margin, fontsize, whitetomove, blacktomove; 
...
struct {
    ...
    unsigned int whitetomove : 1;
    unsigned int blacktomove : 1;
...
put_move_indicator ()
{
    ...
    if ((whitetomove && !flags.reverse) || (blacktomove && flags.reverse)) {
        cy += 7*units;
        dy = -dy;
    }
    if (whitetomove) {
...
put_coords ()
{
    ...
    for (int file = 0; file < 8; ++file)
        printf("<text x=\"%d\" y=\"%d\" fill=\"#000\" class=\"small\">%c</text>\n", 
            file_x0+units*file, file_y0, files[flags.reverse ? 7-file : file]);
    for (int rank = 0; rank < 8; ++rank)
        printf("<text x=\"%d\" y=\"%d\" fill=\"#000\" class=\"small\">%d</text>\n", 
            rank_x0, rank*units+rank_y0, flags.reverse ? rank+1 : 8-rank);
...
build_board (const char *fen)
{
    ...
    board = 8*units;
	int step = flags.reverse ? -units : units;
    ...
    for ( ; i < strlen(fen); ++i)
    {
        if (isalpha(fen[i])) {
            ...
            x = flags.reverse ? 7*units + margin : margin;
            y += step;
        } else if (fen[i] == ' ')
            break;
        else
...
print_usage()
{
    fprintf(stderr, "Usage: svgboard [-c] [-g] [-h] [-p <pieces>] [-r]\n");
